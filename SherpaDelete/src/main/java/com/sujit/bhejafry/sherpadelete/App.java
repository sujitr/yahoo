 ...
 * and any value in the second column. The column separator is "|" symbol for the input file.
...
public class App {
	...
	public static void main( String[] args ) {
    	...
    	String sherpaTableName = hostNamePart.substring(hostNamePart.indexOf("delete/")+7, hostNamePart.length()-1);
        try {
		...
		if(errorFile.length()==0){
			...
			sendMail("<h3>Sherpa Delete Script has finished execution on table '"+sherpaTableName+"'</h3><br><p>No errors have been reported. All tribune media alt-src entries in the attached input file has been deleted.<br>Attached zip file contains all the TMS alternate keys which were deleted from the table.</p>", "TMS ID alt-src key deletion status from "+sherpaTableName, null, fileAttList);
		}else{
			List<String> inputFiletoZip = new ArrayList<String>();
			...
			List<String> fileAttList = new ArrayList<String>();
			fileAttList.add(zipFilePath);
			sendMail("<h3>Sherpa Delete Script has finished execution on table '"+sherpaTableName+"'</h3><br><p>However some errors have been encountered while deleteing records. TMS alt-src key's which failed are available at 'SherpaDeleteFailureLog.txt', inside the attached zip file, along with the input file which contains all the TMS ID alt-src keys which were provided.<br>You might need to retry the failed records for proper delete</p> ", "TMS ID alt-src key deletion status from "+sherpaTableName, null, fileAttList);
		}
    }
    ...
    public static void sendMail(String bodyPart, String sub, String cclist, List<String> fileAttList){
		try
	    {
	      ...
	      String body=bodyPart+"<br>Do not reply to this message  - EOM";
    ...
    public static int intervalNotify(Calendar startTime, Calendar flagTime, String information){
		int status = 0;
		...
		if(diffInSeconds>=900){
			sendMail("<p>Deletion is still going on for Sherpa Table - '"+information+"'</p>", "TMS ID alt-src key deletion status from "+information+" - still going on", null, null);
			status = 1;
		}
		return status;
	}
